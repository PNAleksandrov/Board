...
def notify_reply_on_post(sender, instance, created, **kwargs):
    if created:
        ...
        send_mail(
            ...
            message=f"Отклик: {instance.text}; user:{instance.user.username};"
                    f"http://127.0.0.1:8000/posts/{instance.post.pk}",
...
def notify_reply_on_post(sender, instance, created, **kwargs):
    if instance.accept:
        ...
        send_mail(
            ...
            message=f"Отклик: {instance.text}; был принят автором поста '{instance.post.author.username}';"
                    f"http://127.0.0.1:8000/posts/{instance.post.pk}",
            from_email=DEFAULT_FROM_EMAIL,
...
class ReplyFilterList(LoginRequiredMixin, ListView):
    template_name = 'reply_filter.html'
    ordering = '-date'
